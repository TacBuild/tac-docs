...
Once a message is created, sequencers take over. They store these messages in their local databases and perform comprehensive validation. This includes verifying that all asset transfers match the message parameters exactly, ensuring the integrity of the cross-chain operation.
</Step>
...
Once validated through consensus, messages move to execution on the target chain. This final stage includes all necessary asset operations, such as minting or unlocking tokens, followed by the actual method calls on target contracts.
</Step>
</Steps>
