...
TAC is a Network Extension for TON that expands its compatibility with the Ethereum Virtual Machine (EVM). It enables users to access EVM applications directly through their TON wallets, eliminating the need for managing multiple wallets or using traditional bridges.
...
The EVM equivalence in TAC is obtained by running a fully-fledged EVM Layer 1 based on CosmosSDK + Ethermint, secured by DPoS and Babylon Bitcoin Staking. This TAC EVM Layer is where EVM code is deployed and executed to power TON native-like dApps in the TAC ecosystem.
...
The network's security on both the TAC EVM Layer and TON Adapter relies on real economic stake in the system. These groups of validators (EVM) and sequencers (Adapter) must stake collateral and actively participate in validation rounds. Good performance earns rewards, while incorrect operations face penalties. This economic model aligns incentives across the network, ensuring participants work to maintain security and efficiency.
...
TAC is a CosmosSDK + Ethermint Layer 1 secured by Tendermint-Core and Babylon Bitcoin Staking. As such, the TAC EVM Layer inherits all the security aspects of Ethermint Cosmos chains.
